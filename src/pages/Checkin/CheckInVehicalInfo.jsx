...
const CheckInVehicalInfo = () => {
  const navigate = useNavigate();
  const { t } = useTranslation();
  const [numDigits] = useState(8);
  // Initialize vehicleNum from sessionStorage or empty string
  const [vehicleNum, setVehicleNum] = useState(() => {
    const savedVehicleNum = sessionStorage.getItem("vehicleNum");
    return savedVehicleNum || "";
  });
  const [vehicleDetails, setVehicleDetails] = useState(() => {
    const savedVehicleDetails = sessionStorage.getItem("vehicleDetails");
    return savedVehicleDetails
      ? JSON.parse(savedVehicleDetails)
      : {
          make: "",
          model: "",
          color: "",
  ...
  // Update sessionStorage whenever vehicleNum changes
  useEffect(() => {
    sessionStorage.setItem("vehicleNum", vehicleNum);
    setDisabled(vehicleNum.length !== numDigits);
  }, [vehicleNum, numDigits]);
  ...
  const handleContinue = () => {
    ...
    sessionStorage.setItem("vehicalNumSubmitted", "true");
    // vehicleDetails are already being stored via useEffect
    navigate("/check-in/addon");
  };

  // Function to clear session data if needed
  const clearSessionData = () => {
    sessionStorage.removeItem("vehicleNum");
    sessionStorage.removeItem("vehicleDetails");
    sessionStorage.removeItem("vehicalNumSubmitted");
    setVehicleNum("");
    setVehicleDetails({
      make: "",
      model: "",
    ...
    <>
        ...
        >
          <div className="row g-3">
            <div className="col-md-6">
              <h3 className="input-heading mb-2 d-block">{t("Make")}</h3>
      ...
      >
        ...
        <Modal.Body>
          <div className="py-3">
            <h3 className="text-start input-heading mb-1">
              {makeDropdownOpen ? t("Make") : t("Color")}
            </h3>
...
export default CheckInVehicalInfo;
